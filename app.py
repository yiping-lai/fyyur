...
def search_venues():
  ...
  for match in matching:
    ...
    d['id']=match.id
    d['name']=match.name
    d["num_upcoming_shows"]=0
    venue_shows=Show.query.filter_by(venue_id=match.id).all()
    for x in venue_shows:
      if x.start_time>=datetime.datetime.now():
        d["num_upcoming_shows"]+=1
...
def search_artists():
  ...
  for match in matching:
    ...
    d['name']=match.name
    d["num_upcoming_shows"]=0
    venue_shows=Show.query.filter_by(artist_id=match.id).all()
    for x in venue_shows:
      if x.start_time>=datetime.datetime.now():
        d["num_upcoming_shows"]+=1
    response['data'].append(d)
...
def shows():
  ...
  for show in shows:
    ...
    d['venue_name']=Venue.query.get(show.venue_id).name
    artist=Artist.query.get(show.artist_id)
